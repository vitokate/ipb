...
class _SupportRequest
{
	...
	public function handleForm( $values )
	{
		if ( \IPS\Settings::i()->contact_type == 'contact_nexus_department' )
		{
			...
			if ( isset( $values['contact_referrer'] ) AND $values['contact_referrer'] )
			{
				$content .= \IPS\Theme::i()->getTemplate( 'system', 'core', 'front' )->contactFormReferrer( $values['contact_referrer'] );
			}
			...
			$request->replies = 1;
			$request->title = ( isset( $values['contact_name'] ) AND $values['contact_name'] ) ? sprintf( \IPS\Member::loggedIn()->language()->get( 'contact_nexus_title_with_name' ), $values['contact_name'] ) :  \IPS\Member::loggedIn()->language()->get( 'contact_nexus_title' );

	...
	\IPS\Content\FuturePublishing
{
	 ...
	 * @return bool
	 */
	public function isHotTopic() : bool
	{
		/* Classic installations use popular_time */
		if( !\IPS\CIC )
		{
			return ( $this->popular_time !== NULL and $this->popular_time > time() );
		}
		/* Don't use trending data for new topics */
		elseif( \IPS\CIC AND ( $this->mapped('date') > \IPS\DateTime::create()->sub( new \DateInterval( 'PT1H' ) )->getTimestamp() OR $this->mapped('num_comments' ) <= 2 ) )
		{
			return FALSE;
		}

		/* Never a hot topic if it has no replies */
		if( $this->posts <= 1 )
		{
			return false;
		}

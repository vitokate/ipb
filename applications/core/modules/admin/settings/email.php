...
class _email extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		if ( $values = $settingsForm->values() )
		{
			...
			if( $sendTestEmail )
			{
				$email			= \IPS\Email::buildFromContent( \IPS\Member::loggedIn()->language()->get('email_test_subject'), \IPS\Member::loggedIn()->language()->addToStack('email_test_message'), \IPS\Member::loggedIn()->language()->addToStack('email_test_message'), \IPS\Email::TYPE_BULK );
		...
		/* Build the test form */
		$testFormHtml = '';
		if ( !\IPS\DEMO_MODE AND \IPS\Member::loggedIn()->hasAcpRestriction( 'core', 'settings', 'email_test' ) )
		{
			...
			if ( $values = $testForm->values() )
			{
				try
				{
					$email = \IPS\Email::buildFromContent( $values['subject'], $values['message'], $values['message'], \IPS\Email::TYPE_BULK );
				...
				catch ( \Exception $exception )
				{
					...
					$messageHtml = \IPS\Theme::i()->getTemplate( 'global', 'core', 'global' )->message( $content ?: 'email_test_error_auto', 'error' );
				}
			}

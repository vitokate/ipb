...
class _Categories extends \IPS\Node\Model implements \IPS\Node\Permissions
{
	...
	public static function furlNameIsTaken( string $furl, int $databaseId, ?int $excludeId=null ) : bool
	{
		...
		foreach ( (new \IPS\Patterns\ActiveRecordIterator( \IPS\Db::i()->select( '*', 'cms_database_categories', $where ), self::class )) as $cat )
		{
			if ( ( !empty( $cat->furl_name ) and $cat->furl_name == $furl ) or ( empty( $cat->furl_name ) AND !empty( $cat->name ) AND \IPS\Http\Url\Friendly::seoTitle( $cat->name ) == $furl ) )
			{
				return true;
			}
		}
	...
	public function formatFormValues( $values )
	{
		...
		{
			$ok  = static::furlNameIsTaken( \IPS\Http\Url\Friendly::seoTitle( $name ), $this->database_id, $this->id ?: null );
			...
			$values['furl_name'] = $ok ? \IPS\Http\Url\Friendly::seoTitle( $name ) : $this->id . '_' . \IPS\Http\Url\Friendly::seoTitle( $name );
		}
	...
	public function permissions()
	{
		...
		if ( !\IPS\Dispatcher::hasInstance() OR \IPS\Dispatcher::i()->controllerLocation !== 'front' OR $this->isClubCategory() )
		{
	 ...
	 */
	public function get__club() : \IPS\Member\Club|null
	{
	...
	public function canManagePermissions()
	{
		...
		if ( $this->isClubCategory() and !( \IPS\Dispatcher::i() instanceof \IPS\Dispatcher\Front ) )
		{
			return false;
		}
		else if ( $this->isClubCategory() )
		{
			return $this->can( 'add' );
		}
	...
	public function can( $permission, $member=NULL, $considerPostBeforeRegistering = TRUE )
	{
		...
		if ( $club = $this->_club )
		{
			if ( \in_array( $permission, [ 'add', 'edit', 'delete', 'reply', 'review' ] ) and $_member instanceof \IPS\Member )
			{
			...
			if ( \in_array( $permission, ['view', 'read' ] ) )
			{
				...
				if ( $_member instanceof \IPS\Member and $_member->modPermission('can_access_all_clubs') )
				{
					return TRUE;
				}

				/* If it's not approved, only moderators and the person who created it can see it */
				if ( \IPS\Settings::i()->clubs_require_approval and !$club->approved and $_member instanceof \IPS\Member )
				{
					return ( $_member->modPermission('can_access_all_clubs') or ( $club->owner AND $_member->member_id == $club->owner->member_id ) );
				}

				/* Owner or leader? */
				if ( $_member instanceof \IPS\Member and ( $_member->member_id === $club->owner->member_id OR $club->memberStatus( $_member ) === \IPS\Member\Club::STATUS_LEADER ) )
				{
					return TRUE;
				}
				...
				if ( $this->club_category_meta_index and $club->type !== \IPS\Member\Club::TYPE_PRIVATE and \in_array( 'nonmember', $viewGroups ) )
				{
				...
				/* Nonmembers */
				if ( \in_array( 'nonmember', $viewGroups ) and $_member instanceof \IPS\Member and $_member->member_id )
				{
					return true;
				}
			
				/* Moderators? */
				if ( \in_array( 'moderator', $viewGroups ) AND $club->memberStatus( $_member ) === \IPS\Member\Club::STATUS_MODERATOR )
				{
					return TRUE;
				}
	...
	public function isClubCategory() : bool
	{
		...
		{
			$database = \IPS\cms\Databases::load( $this->database_id );
			$class = '\IPS\cms\Categories' . $this->database_id;
			if ( $database->allow_club_categories and \IPS\IPS::classUsesTrait( $class, 'IPS\Content\ClubContainer' ) )
			{
				return true;
			}
		}
		catch ( \OutOfBoundsException|\OutOfRangeException ) { }

...
{	
	...
	public function init()
	{
		...
		if ( \count( $itemIndexIds ) and \IPS\Settings::i()->search_method == 'mysql' )
		{
			...
			{	
				$orderBy = ' ORDER BY index_tag ASC';
			}
			
			$this->tags = iterator_to_array( \IPS\Db::i()->select( 'index_id, GROUP_CONCAT(index_tag' . $orderBy . ') as index_tags', 'core_search_index_tags', array( array( \IPS\Db::i()->in( 'index_id', $itemIndexIds ) ), array( 'index_is_prefix = 0' ) ), NULL, NULL, 'index_id' )->setKeyField('index_id') );

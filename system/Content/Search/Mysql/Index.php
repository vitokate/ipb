...
class _Index extends \IPS\Content\Search\Index
{
		...
		if( $indexData )
		{
			...
			else
			{
				...
				else if ( $indexData['index_is_last_comment'] and $indexData['index_item_id'] )
				{
					...
					\IPS\Db::i()->update( 'core_search_index', array( 'index_date_updated' => $indexData['index_date_updated'], 'index_date_commented' => $indexData['index_date_commented'] ), array( \IPS\Db::i()->in( 'index_class', $classes ) . ' AND index_item_id=? and index_object_id=?', $indexData['index_item_id'], ( $firstCommentId ?? $indexData['index_item_id'] ) ) );
				}
	 ...
	 * @return	string
	 */
	public function getIndexId( \IPS\Content\Searchable $object ): string
	{
		$databaseColumnId = $object::$databaseColumnId;
	...
	public function removeFromSearchIndex( \IPS\Content\Searchable $object )
	{
		...
		if ( $object instanceof \IPS\Content\Comment )
		{
			...
			try
			{
				$this->resetLastComment( $classes, $object->mapped('item'), $object->$idColumn, $this->getFirstCommentId( $object ) );
			}
			catch( \Exception $ex ) { }
	 ...
	 * @return 	void
	 */
	public function resetLastComment( $classes, $indexItemId, $ignoreId = NULL, $firstCommentId = NULL )
	{

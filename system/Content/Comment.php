...
abstract class _Comment extends \IPS\Content
{
	...
	public function onUnhide( $approving, $member )
	{
		...
		try
		{
			if ( $item->container()->_comments !== NULL )
			{
				$item->container()->setLastComment( $this, $this->item() );
				$item->container()->_comments = ( $item->container()->_comments + 1 );
				$item->container()->resetCommentCounts();
				$item->container()->save();
			}
	...
	public function delete()
	{
		...
		try
		{
			/* Don't fetch the current PID if we're using R/W separation where there may be a replication delay */
			$lastComment = $item->comments( 1, 0, 'date', 'desc', NULL, FALSE, NULL, [ static::$databaseTable . '.' . static::$databasePrefix . $idColumn . ' != ?', $this->$idColumn ], TRUE );
		}
